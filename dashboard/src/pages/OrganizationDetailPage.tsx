...
export function OrganizationDetailPage() {
  ...
  const orgQuery = useQuery({
    ...
    queryFn: () => platformService.getOrganization(id),
    enabled: Boolean(id),
  });

  const usersQuery = useQuery({
    queryKey: queryKeys.organizationUsers(id, usersPage),
    queryFn: () => platformService.listOrganizationUsers(id, { page: usersPage, limit: 25 }),
    enabled: Boolean(id),
  ...
  const changePlan = useMutation({
    ...
    // A downgrade below current usage is refused by the API with a 409 that
    // names the numbers — surface it rather than a generic failure.
  ...
  const toggleUser = useMutation({
    mutationFn: ({ userId, isActive }: { userId: string; isActive: boolean }) =>
      platformService.setUserActive(id, userId, isActive),
    onSuccess: (user) => {
      toast.success(`${user.name} ${user.isActive ? 'reactivated' : 'deactivated'}`);
      void queryClient.invalidateQueries({
        queryKey: ['platform', 'organization', id, 'users'],
      });
      invalidateOrg();
    },
    onError: (error) => toast.error(errorMessage(error)),
  ...
  if (orgQuery.isError) {
    return (
      <ErrorState
        message={errorMessage(orgQuery.error)}
          ...
          {orgQuery.isPending ? (
            ...
            <>
              <div className="flex flex-wrap items-center gap-2.5">
                <h1 className="text-xl font-semibold text-[var(--text)]">{org?.name}</h1>
                {org && <StatusChip status={org.status} />}
              </div>
      ...
      {suspended && org?.suspendedReason && (
        <Card className="border-[var(--danger-border)] bg-[var(--danger-surface)] px-5 py-3.5">
          <p className="text-[13px] text-[var(--text)]">
            ...
            {org.suspendedAt && ` on ${formatDate(org.suspendedAt)}`} — {org.suspendedReason}
          </p>
        </Card>
      )}
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          ...
          <dl className="grid grid-cols-1 gap-x-6 gap-y-4 p-5 sm:grid-cols-2">
            ...
            <Fact label="Data tier">
              <span className="text-[13px] capitalize text-[var(--text)]">
                {org?.tier ?? '—'}
              </span>
            </Fact>

            <Fact label="Users">
              <span className="tabular text-[13px] text-[var(--text)]">
                {formatNumber(org?.counts.activeUsers)} active
                {org && org.limits.maxUsers !== -1 && (
                  <span className="text-[var(--text-subtle)]">
                    {' '}
                    of {org.limits.maxUsers} allowed
                  </span>
                )}
              </span>
            </Fact>
            ...
            <Fact label="Leads">
              <span className="tabular text-[13px] text-[var(--text)]">
                {formatNumber(org?.counts.leads)}
                {org && org.limits.maxLeads !== -1 && (
                  <span className="text-[var(--text-subtle)]">
                    {' '}
                    of {formatNumber(org.limits.maxLeads)}
                  </span>
                )}
              </span>
            </Fact>
            ...
            {org?.trialEndsAt && (
              <Fact label="Trial ends">
                <span className="text-[13px] text-[var(--text)]">
                  ...
                  <span className="block text-[12px] text-[var(--text-muted)]">
                    {formatRelative(org.trialEndsAt)}
                  </span>
                </span>
              </Fact>
            )}
        ...
        <Card>
          ...
          <div className="space-y-3 p-5">
            <Textarea
              ...
              placeholder="Context the next person on this account will want…"
              rows={6}
              disabled={!can('orgs.update')}
            />
